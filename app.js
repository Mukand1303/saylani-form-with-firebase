...
submitBtn.addEventListener("click", async () => {
  ...
  const fatherCnic = fatherCnicInput.value || ""; // Simplified the fatherCnic assignment
  ...
  if (!city) {
    ...
    try {
      // Display a loading spinner while the data is being submitted
      submitBtn.innerHTML = `
        <div class="spinner-border" role="status">
          <span class="visually-hidden">Loading...</span>
        </div>
      ...
      // Use setDoc to save the data to Firestore
      await setDoc(doc(db, "students", cnic), studentData);

      // Reset the button text after successful submission
      ...
      haveLaptop.value = "";
      // Assuming profilePicDiv is the ID of the element to hide
